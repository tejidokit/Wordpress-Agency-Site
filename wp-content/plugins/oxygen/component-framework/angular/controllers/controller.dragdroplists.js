...
CTFrontendBuilder.controller("ControllerDragnDropLists", function($scope, $timeout, $parentScope, $interval) {
    ...
    $scope.dragoverCallback = function(id, event, external, type) {
        ...
        // do not allow drop inside a sibling clone in case of a dynamic list
        if($scope.selectedDragElement.closest('.oxy-dynamic-list > *').length && angular.element(event.target).closest('.oxy-dynamic-list > *').length
            && ($scope.selectedDragElement.closest('.oxy-dynamic-list > *').attr('id') == angular.element(event.target).closest('.oxy-dynamic-list > *').attr('id'))
            ) {
            if($scope.selectedDragElement.closest('.oxy-dynamic-list > *').get(0) !== angular.element(event.target).closest('.oxy-dynamic-list > *').get(0)) {
                return false;
            }
        }

        // in case its inside a dynamic list, then select the correct item
        if($scope.dynamicListActivatedIndex !== null && $scope.selectedDragElement.length > 1) {
            $scope.currentDragoverElement = angular.element($scope.currentDragoverElement.get($scope.dynamicListActivatedIndex));
        }
    ...
    $scope.dragstartCallback = function(componentId, selector, event) {
        ...
        $scope.dragSrcOxyList = $scope.selectedDragElement.closest('.oxy-dynamic-list');
        
        if($scope.dynamicListActivatedIndex !== null && $scope.selectedDragElement.length > 1) {
            $scope.selectedDragElement = angular.element($scope.selectedDragElement.get($scope.dynamicListActivatedIndex));
        }
    ...
    $scope.dragendCallback = function(orderNum, componentName, event) {
        ...
        if(endParentID != undefined) {
            ...
            if($scope.dragSrcOxyList.length > 0 && (oxyList.length < 1 || $scope.dragSrcOxyList.attr('ng-attr-component-id') != oxyList.attr('ng-attr-component-id'))) {
                setTimeout(function() {
                    $scope.rebuildDOM(parseInt($scope.dragSrcOxyList.attr('ng-attr-component-id')));    
                }, 100);
            }
    ...
    $scope.dragstartResizeBoxCallback = function(event,componentId) {
        ...
        if($scope.dynamicListActivatedIndex !== null && component.length > 1) {
            component = angular.element(component.get($scope.dynamicListActivatedIndex));
        }
