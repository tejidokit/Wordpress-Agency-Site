...
CTFrontendBuilderUI.controller("ControllerUI", function($controller, $anchorScroll, $location, $scope, $timeout, $interval, $window, dragulaService, $compile, ctScopeService) {  
    ...
    $scope.disableContentEdit = function() {
            ...
            if ($scope.iframeScope.component.active.name != 'ct_span') {
                ...
                if(oxyDynamicList.length > 0) {
                    ...
                    // } else {
                        ...
                        // $scope.iframeScope.buildComponentsFromTree($scope.iframeScope.componentsTree.children, $scope.iframeScope.component.active.id, false, container );
                        
                        // increment id
                        $scope.iframeScope.component.id+=idIncrement;

                        var originalComponent = activeComponent.next();
                        originalComponent.css('display', '').removeAttr('disabled');
        ...
        if(oxyDynamicList === null || oxyDynamicList.length < 1) {
    ...
    $scope.radioButtonClick = function(componentName, paramName, paramValue) {
        ...
        if(componentName=='oxy_dynamic_list' && paramName=='acf_repeater') {
            $scope.acfRepeaterDynamicDialogProcess();
        }
        ...
        if( componentName == "ct_modal" ) {
            if( typeof $scope.currentModal === 'undefined' ) {
                ...
                $scope.currentModal != $scope.iframeScope.component.active.id &&
                typeof $scope.iframeScope.component.options[$scope.iframeScope.component.active.id] !== 'undefined' &&
                $scope.iframeScope.component.options[$scope.iframeScope.component.active.id].name == "ct_modal" ) {

                $scope.iframeScope.setOptionModel( "behavior", "1", $scope.currentModal );
                $scope.currentModal = $scope.iframeScope.component.active.id;
            }
        }
    }
    ...
    $scope.acfRepeaterDynamicDialogProcess = function(id) {
        ...
        setTimeout(function() {
            ...
            if(repeater && repeater != '') {
                ...
                $scope.iframeScope.dynamicShortcodesContentMode[0]['children'].push(
                    {
                        name: "Repeater Field", 
                        data:"acfreparray", 
                        properties: [
    ...
    $scope.evalGlobalConditions = function(id) {
        ...
        if(activeComponent) {
          ...
          if(oxyDynamicList.length > 0) {
              ...
              $scope.iframeScope.dynamicListAction(listId, id);
              return;
          }
